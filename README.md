...
Every produced artifact now passes a fail-closed security decision before storage and release. The baseline scanner re-verifies artifact integrity and rejects the standard antivirus test signature or embedded private-key material. The completed build records the decision, scanner identity, and timestamp; release creation accepts only an explicitly approved result. This establishes the replacement boundary for a production malware, dependency-vulnerability, and SBOM scanner—the baseline scanner is not a substitute for those production controls.
...
Local acceptance: set `VCP_E2E_TRIVY_IMAGE` to an approved scanner digest and `VCP_E2E_TRIVY_NETWORK` to a disposable network, then run `pnpm exec vitest run test/trivy-live.test.ts`. This uses a synthetic vulnerable Lodash lockfile, verifies that the SBOM identifies Lodash, and expects policy rejection. It skips without explicit configuration. Full-source scanning reports dependencies that may not ship in a bundled application; it is not an exact inventory of the final executable. Production malware scanning, signed attestations, exceptions, private-registry authentication, and scan-database provenance/freshness reporting remain follow-up work.

The runtime shares Trivy database files under `<SUPPLY_CHAIN_EVIDENCE_ROOT>/scanner-cache`; set `TRIVY_CACHE_ROOT` to override. Trivy's database update checks remain enabled; scan artifacts use the in-memory backend and prior passing decisions are not reused. Restrict this directory to the scanner/operator. On Linux, provision it writable by container UID/GID 65532. An atomic `.vcp-scan-lock` directory serializes workers sharing this cache; acquisition times out after five minutes. An abandoned lock requires operator investigation and manual removal only after verifying that no scanner owns it. Multi-host storage/locking behavior is not validated.

Authenticated evidence endpoints are `GET /companies/:companyId/applications/:applicationId/security-scans` and `GET .../security-scans/:scanId`. Both enforce company access and scope reports to the application's build/release records. The portal's selected application shows saved passing and blocked scans, findings, and a full CycloneDX download. These are historical results, not current health guarantees. Storage is still operator-managed: each report is limited to 17 MiB on read, the flat index is capped at 10,000 entries, and retention/archival must be managed before that limit. Signed evidence and a scalable database index remain future work.

Run browser coverage with `pnpm test:portal` after installing a Playwright Chromium browser, or set `PLAYWRIGHT_CHANNEL=msedge` for installed Edge. Browser tests use synthetic mocked API responses; backend tenant authorization is tested separately.
