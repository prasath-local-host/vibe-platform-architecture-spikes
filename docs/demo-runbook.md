...
The deployment adapter verifies image-tar SHA-256, the exact source tag, and the approved image configuration digest. A build's image ID may identify that configuration or an OCI manifest/index referencing it. After loading, the adapter resolves the local Docker ID and re-exports that immutable ID to verify the exact configuration bytes, including runtime settings and ordered filesystem-layer digests. Docker verifies layer contents during load. This supports classic and containerd image stores without trusting the tag alone or substituting an unverified ID. Containers run by the verified local ID, recorded separately as `local_image_id`; the original approved manifest remains unchanged for Stage/Prod promotion. Reserve space for one additional image archive in the runner workspace during verification; that temporary copy is removed on success or failure.

The adapter checks a candidate on a temporary loopback port before replacing the stable binding. Switching requires stopping/recreating a container and can cause brief downtime; this is not zero-downtime ingress. A failed replacement attempts to restore the previous container and verifies it. Application rollback does not reverse database changes. Previous containers remain for recovery; there is no automatic image/container retention cleanup yet.

After a deployment-helper fix, start a new build on the updated platform branch, then deploy the new successful build run ID. Deployment deliberately checks out the platform revision recorded by the build; rerunning an old candidate still uses the old helper. Do not edit `release.json` or bypass digest verification to work around image-ID differences.
