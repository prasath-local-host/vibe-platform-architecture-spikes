...
export class PostgresReleaseRepository implements ReleaseRepository {
  ...
  async rolledBack(releaseId: string, deploymentUrl: string, error: string, occurredAt: string, event: AuditEvent) { await this.finish(releaseId, { status: "rolled-back", deployment_url: deploymentUrl, health_verified_at: occurredAt, completed_at: occurredAt, locked_by: null, error }, event); }
