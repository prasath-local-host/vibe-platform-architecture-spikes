...
describe.skipIf(!image)("release routing E2E", () => {
  ...
  it("deploys healthy, switches traffic, rejects unhealthy, and restores the route", async () => {
    ...
    for (const [healthy, digit] of [[true, "2"], [false, "3"]] as const) {
      ...
      builds.set(buildId, { id: buildId, companyId, applicationId, repositoryUrl: "https://example.invalid/platform-fixture", sourceRevision: digit.repeat(40), packageManager: "npm", script: "build", idempotencyKey: `build-${digit}`, correlationId: `correlation-${digit}`, status: "completed", attempts: 1, result: { artifactId: artifact.id, artifactDigest: artifact.digest, restorationStatus: "succeeded", buildStatus: "succeeded" }, createdAt: new Date().toISOString(), completedAt: new Date().toISOString() });
    }
