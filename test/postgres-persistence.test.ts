...
describe.skipIf(!databaseUrl)("PostgreSQL persistence", () => {
  ...
  it("persists idempotent build jobs and completes one concurrent claim", async () => {
    ...
    const engine = { async execute() { return { artifactId: "33333333-3333-4333-8333-333333333333", artifactDigest: `sha256:${"a".repeat(64)}`, restorationStatus: "succeeded" as const, buildStatus: "succeeded" as const, securityStatus: "approved" as const, securityScanner: "test-scanner/1", securityScannedAt: new Date().toISOString() }; } };
